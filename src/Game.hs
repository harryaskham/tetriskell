...
nextPiece game = fst . randomPieceAtTop $ game ^. pieceGen
...
flushCompleted game = flushed & score %~ (+ linesCleared)
  where
    ...
    linesCleared =
      numPopulatedRows (game ^. grid) - numPopulatedRows (flushed ^. grid)
...
move Hold game =
  ...
  where
    pieceToHold = pieceAtTop $ game ^. piece
